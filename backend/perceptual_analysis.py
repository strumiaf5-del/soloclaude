...
def _safe_metric(value, default: float) -> float:
    """F2.3: sanitiza NaN/Inf en métricas técnicas.

    Si el valor no es finito (NaN o ±Inf) o no se puede convertir a float,
    devuelve `default`. Esto evita que comparaciones como
    `centroid > IN_YOUR_FACE_CENTROID_HZ` se comporten de forma
    impredecible (NaN > x es False → cae en la rama "balanced").
    """
    try:
        f = float(value)
...
def analyze_perceptual_profile(analysis: Dict) -> PerceptualProfile:
    ...
    # F2.3: sanitizamos las métricas clave contra NaN/Inf antes de usarlas en
    ...
    plr = _safe_metric(analysis.get("plr_db", 8), 8.0)
    lra = _safe_metric(analysis.get("lra", 4), 4.0)
    ...
    presence_band = band_energies.get("presence", -20.0)  # 2k-4k
    ...
    lufs = _safe_metric(analysis.get("lufs", -14), -14.0)
    ...
    subs_energy = band_energies.get("sub_bass", -20.0)
    mids_energy = band_energies.get("mid", -20.0)
    air_energy_val = band_energies.get("air", -20.0)
    ...
    true_peak = _safe_metric(analysis.get("true_peak_db", -3), -3.0)
    clipping_ratio = _safe_metric(analysis.get("clipping_ratio", 0.0), 0.0)
...
def get_genre_from_perceptual(profile: PerceptualProfile, 
    ...
    # no caigan en silencio por valores no finitos.
    lufs = _safe_metric(analysis.get("lufs", -14), -14.0)
    plr = _safe_metric(analysis.get("plr_db", 8), 8.0)
    lra = _safe_metric(analysis.get("lra", 4), 4.0)
    transient_density = _safe_metric(analysis.get("transient_density", 0.5), 0.5)
    centroid = _safe_metric(analysis.get("spectral_centroid_hz", 3000), 3000.0)
