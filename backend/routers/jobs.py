...
def _report_payload(job_id: str, job: dict) -> dict:
    ...
    report = {
        ...
        "params": job["params"],
        "analysis_before": result.get("analysis_before"),
        "analysis_after": result.get("analysis_after"),
        "mix_advice_before": result.get("mix_advice_before"),
        "mix_advice_after": result.get("mix_advice_after"),
        "recommendations_before": result.get("recommendations_before"),
        "recommendations_after": result.get("recommendations_after"),
        "chain_meters": result.get("chain_meters", {}),
    }
    if "reference_match" in result:
        report["reference_match"] = result["reference_match"]
        report["analysis_reference"] = result["analysis_reference"]
    return report
...
def create_jobs_router(*, jobs, sanitize_track_name, processed_dir: Optional[str] = None) -> APIRouter:
    ...
    def download(job_id: str, name: Optional[str] = Query(None, description="Nombre del tema para el archivo descargado")):
        ...
        if not output_path:
            raise HTTPException(410, "Result not available")
