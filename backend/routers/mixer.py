...
async def mix_upload_stem(
    ...
    """
    # B-S3: validate_audio_file() se llama ANTES del open() (magic bytes +
    # extensiones). Pero el filename y el stem_name vienen del cliente y podían
    # contener "/" o ".." — un atacante armaba un path con ".." para escapar de
    # UPLOAD_DIR y escribir/sobrescribir cualquier archivo del host (p.ej.
    # users_db.json, .env). Sanitizo el stem_name y confino el path final con
    # realpath + startswith contra el directorio real de uploads.
    ...
    if not ext:
        ext = ".wav"
    stem_name = stem_name.strip() if stem_name and not any(c in stem_name for c in "/\\..\x00") else ""
    name = stem_name or base
    path = os.path.realpath(os.path.join(UPLOAD_DIR, f"mix_{session_id}_{name}{ext}"))
    if not path.startswith(os.path.realpath(UPLOAD_DIR)):
        raise HTTPException(400, "Invalid stem path")
