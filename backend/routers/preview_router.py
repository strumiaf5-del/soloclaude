    ...
    async def create_source_snapshot(
        ...
        try:
            if library_id:
                ...
                if content_type not in _PREVIEW_ALLOWED_MIME_TYPES:
                    raise HTTPException(
                        415,
                        f"Tipo MIME no soportado: '{content_type}'. Permitidos: {sorted(_PREVIEW_ALLOWED_MIME_TYPES)}",
                    )
                data = await read_and_validate(file)
                if len(data) > MAX_FILE_SIZE:
                    raise HTTPException(
                        413,
                        f"Archivo demasiado grande. Máximo: {MAX_FILE_SIZE // 1024 // 1024} MB",
                    )
            ...
            # F10.3: bounds defensivos sobre la meta del snapshot. La Pydantic
            # PreviewSourceResponse solo valida el shape, no rechaza valores
            # absurdos heredados de un audio mal formado (sr=1, duración
            # gigante, etc.) antes de que se persista el preview.
            sample_rate = int(meta.get("sample_rate") or 0)
            duration_sec = float(meta.get("duration_sec") or 0.0)
            if not (8000 <= sample_rate <= 192000):
                raise HTTPException(
                    400,
                    f"sample_rate debe estar entre 8000 y 192000 Hz (recibido: {sample_rate})",
                )
            if not (0.1 <= duration_sec <= 300):
                raise HTTPException(
                    400,
                    f"duration_sec debe estar entre 0.1 y 300 (recibido: {duration_sec})",
                )
