...
def create_reference_library_router(*, reference_library_module, reference_library_dir: str, current_user_dependency) -> APIRouter:
    router = APIRouter()

    @router.get("/reference-library", tags=["Reference Library"], dependencies=[Depends(current_user_dependency)])
    ...
    @router.post("/reference-library/rescan", tags=["Reference Library"], dependencies=[Depends(current_user_dependency)])
