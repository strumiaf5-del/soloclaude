...
"""
    ...
    try:
        ...
        logging.getLogger(__name__).exception("Error llamando a Gemini: %s", e)
        raise HTTPException(500, "Error llamando a Gemini: operación no completada")

    # 4. Sanitizar / Clampear valores
    def _safe_clamp(value, default: float, lo: float, hi: float) -> float:
        try:
            raw = float(value)
            if math.isnan(raw):
                return default
    ...
    for name, params in suggestions.items():
        params["gain_db"] = _safe_clamp(params.get("gain_db", 0.0), 0.0, -18, 18)
        params["hp_cutoff_hz"] = _safe_clamp(params.get("hp_cutoff_hz", 20.0), 20.0, 20, 500)
        params["lp_cutoff_hz"] = _safe_clamp(params.get("lp_cutoff_hz", 20000.0), 20000.0, 2000, 20000)
        params["eq_low_gain_db"] = _safe_clamp(params.get("eq_low_gain_db", 0.0), 0.0, -12, 12)
        params["eq_low_freq"] = _safe_clamp(params.get("eq_low_freq", 100.0), 100.0, 40, 400)
        params["eq_low_q"] = _safe_clamp(params.get("eq_low_q", 0.8), 0.8, 0.3, 4)
        params["eq_lomid_gain_db"] = _safe_clamp(params.get("eq_lomid_gain_db", 0.0), 0.0, -12, 12)
        params["eq_lomid_freq"] = _safe_clamp(params.get("eq_lomid_freq", 500.0), 500.0, 200, 1500)
        params["eq_lomid_q"] = _safe_clamp(params.get("eq_lomid_q", 1.0), 1.0, 0.3, 4)
        params["eq_himid_gain_db"] = _safe_clamp(params.get("eq_himid_gain_db", 0.0), 0.0, -12, 12)
        params["eq_himid_freq"] = _safe_clamp(params.get("eq_himid_freq", 3000.0), 3000.0, 800, 8000)
        params["eq_himid_q"] = _safe_clamp(params.get("eq_himid_q", 1.0), 1.0, 0.3, 4)
        params["eq_high_gain_db"] = _safe_clamp(params.get("eq_high_gain_db", 0.0), 0.0, -12, 12)
        params["eq_high_freq"] = _safe_clamp(params.get("eq_high_freq", 10000.0), 10000.0, 4000, 18000)
        params["eq_high_q"] = _safe_clamp(params.get("eq_high_q", 0.8), 0.8, 0.3, 4)
        params["comp_threshold"] = _safe_clamp(params.get("comp_threshold", 0.5), 0.5, 0.01, 1)
        params["comp_ratio"] = _safe_clamp(params.get("comp_ratio", 4.0), 4.0, 1, 20)
        params["comp_attack_ms"] = _safe_clamp(params.get("comp_attack_ms", 10.0), 10.0, 0.1, 100)
        params["comp_release_ms"] = _safe_clamp(params.get("comp_release_ms", 100.0), 100.0, 10, 500)
        params["comp_makeup_db"] = _safe_clamp(params.get("comp_makeup_db", 0.0), 0.0, -6, 24)
        params["pan"] = _safe_clamp(params.get("pan", 0.0), 0.0, -1, 1)
