...
def _synthesize_ir(sr: int, decay_time: float, size: float = 1.0) -> np.ndarray:
    ...
    # valor mínimo sano (RT60 muy corto) en lugar de crashear.
    if not np.isfinite(decay_time) or decay_time <= 0.0:
        decay_time = 0.1
    # Duración del IR
...
class ReverbProcessor:
    ...
    def __init__(self, sr: int, ir_cache_size: int = 10):
        if not (8000 <= sr <= 192000):
            raise ValueError(f"sr must be between 8000 and 192000, got {sr}")
        self.sr = sr
