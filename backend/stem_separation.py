...
def separate_vocals_hq(audio: np.ndarray, sr: int, progress_cb=None,
    ...
    # (Demucs / RoFormer escriben el WAV de entrada y los stems de salida dentro
    # de este dir). Si STEMS_TMP_DIR está en el .env, gana; si no, fallback
    # al path bajo el workspace.
    stems_tmp_parent = _os.getenv(
        "STEMS_TMP_DIR",
        _os.path.join(_os.path.dirname(_os.path.abspath(__file__)), "..", "processed_stems", "_tmp"),
    )
    _os.makedirs(stems_tmp_parent, exist_ok=True)
    ...
    with tempfile.TemporaryDirectory(dir=stems_tmp_parent) as tmpdir:
